...
def youtube_download(request_user, message):
    try:
        ...
        request = YouTube(f'{URL_YOUTUBE}{check}')
        stream = request.streams.filter(only_audio=True).get_by_itag(140)
        file_music = str(stream.download(output_path=f'data/{message.chat.id}/')).replace(PWD, '')
...
def search_shazam(message):
    try:
        ...
        mp4_file_content_to_recognize = open(f'{SAVE_DIR}{file_name}', 'rb').read()
        shazam = Shazam(
